...
abstract class Tomato_Widget 
{
	...
	public function __construct($module = null, $name = null) 
	{
		/**
		 * Define the widget's module and name based on the class name
		 */
		if ($module == null && $name == null) {
			$class = get_class($this);
			list($module, $widgetDir, $name, $widgetClass) = explode('_', $class);
		}
	...
	public function toTagString()
	{
		...
		$info = Tomato_Widget_Config::getWidgetInfo($this->getModule(), $this->getName());
		...
		foreach ($this->_params as $name => $param) {
			$type  = (is_array($param) && isset($param['type'])) ? $param['type'] : 'default';
			$value = (is_array($param) && isset($param['value'])) ? $param['value'] : $param;
			$data['params'][$name] = array(
				'type'  => $type,
				'value' => $value,
			);
		}
		
		$str = '[x:widget data="' . Zend_Json::encode($data) . '"][/x:widget]';
		return $str;
	}
}
